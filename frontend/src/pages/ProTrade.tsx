...
import { Search, Wallet, Star } from 'lucide-react';
      ...
      <div className="lg:w-72 shrink-0">
        <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-3">
          <div className="relative mb-3">
            ...
            <input type="text" placeholder="Coin ara... (örn: BTC)" value={search} onChange={e => setSearch(e.target.value)}
          ...
          {search.length < 2 && <div className="mb-2 text-xs text-[var(--color-text-muted)]">⭐ Favoriler</div>}
          <div className="space-y-0.5 max-h-[500px] overflow-y-auto">
            {displayCoins.map(([sym, data]: any) => (
              <div key={sym} onClick={() => setSymbol(sym)}
                className={`flex items-center justify-between p-2 rounded-lg cursor-pointer transition-all text-xs ${symbol === sym ? 'bg-[var(--color-accent)]/20 border border-[var(--color-accent)]/30' : 'hover:bg-[var(--color-bg-hover)]'}`}>
        ...
        <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-2">
          <div className="flex items-center justify-between px-2 mb-2">
            ...
            <div className="flex gap-4 text-xs">
              <span className={`${current.change_24h >= 0 ? 'text-green-400' : 'text-red-400'}`}>{current.change_24h >= 0 ? '+' : ''}{current.change_24h?.toFixed(2)}%</span>
      ...
      <div className="lg:w-72 shrink-0">
        ...
        <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-3">
          ...
          <button onClick={handleOrder}
            className={`w-full py-2.5 rounded-lg text-sm font-bold ${side === 'BUY' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'} text-white`}>{side} {symbol}</button>
