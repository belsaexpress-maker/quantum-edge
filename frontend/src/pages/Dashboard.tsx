...
const Dashboard: React.FC = () => {
  ...
  const aiSignals = [
    { s: 'BTC', sig: 'BUY' as const, str: 92, price: 118420, name: 'Bitcoin' },
    { s: 'ETH', sig: 'HOLD' as const, str: 71, price: 4120, name: 'Ethereum' },
    { s: 'SOL', sig: 'SELL' as const, str: 85, price: 235, name: 'Solana' },
  ];
    ...
    <div className="space-y-3 max-w-full overflow-x-hidden">
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-2 -mx-1 px-1">
        {cryptoData.slice(0, 10).map((coin) => (
          <div key={coin.symbol} onClick={() => handleSelectCoin(coin.symbol, coin.name)}
            className={`shrink-0 px-2 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-all ${
              selectedSymbol === `${coin.symbol}USD` ? 'bg-[var(--color-accent)]/20 border border-[var(--color-accent)]/40' : 'bg-[var(--color-bg-card)] border border-[var(--color-border)] hover:border-[var(--color-accent)]/50'
            }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Card className="p-3">
          ...
          {aiSignals.map((x) => (
            <div key={x.s} onClick={() => handleSelectCoin(x.s, x.name)} className="flex items-center justify-between p-2 bg-[var(--color-bg-primary)] rounded-lg mb-1.5 cursor-pointer hover:bg-[var(--color-bg-hover)] transition-colors">
